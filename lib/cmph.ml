...
module Bindings = struct
  ...
  let cmph_io_vector_adapter =
    foreign "cmph_io_vector_adapter" (ptr string @-> returning cmph_io_adapter_t)
...
module KeySet = struct
  ...
  let of_list : string list -> t = fun keys ->
    let arr = CArray.make string (List.length keys) in
    List.iteri (CArray.set arr) keys;
    let ret = {
      keys = arr;
      adapter = Bindings.cmph_io_vector_adapter (CArray.start arr);
    } in
    Gc.finalise
...
module Config = struct
  ...
  let create : ?algo:algo -> KeySet.t -> t =
    fun ?algo keyset ->
      let config = Bindings.cmph_config_new keyset.adapter in
      begin match algo with
      | Some x -> Bindings.cmph_config_set_algo config (algo_value x)
      | None -> ()
      end;
      let ret = { config } in
